...
namespace MarioPort
{
   public static class BackGr
   {
      ...
      private static byte[] BackGrMap = new byte[Max];
      private static ushort[] ColorMap = new ushort[COLORMAP_MAX];
      ...
      public static void DrawBackGr(bool FirstTime)
      {
         ...
         if (Clouds != 0)
         {
            ...
            PutClouds(i, Buffers.XView - Buffers.LastXView[FormMarioPort.formRef.CurrentPage()]);
         }
      }
      ...
      public static void DrawBackGrMap(int Y1, int Y2, int Shift, byte C)
      {
         ...
         for (int i = 0; i <= 320 - 1; i++)
         {
            for (j = Y1 - BackGrMap[i + Shift]; j <= Y2; j++)
               if (FormMarioPort.formRef.GetPixel(i, j) >= 0xC0)
                  FormMarioPort.formRef.PutPixel(i, j, C);
         }
      }
      ...
      public static void ReadColorMap()
      {
         ...
         {
            ColorMap[i] = (ushort)(FormMarioPort.formRef.GetPixel(Buffers.XView + Shift, i) * 256 + FormMarioPort.formRef.GetPixel(Buffers.XView + Shift + 1, i));
         }
      }
      ...
      {
         if (((X / W) % 3) == 0)
            FormMarioPort.formRef.PutImage(X, Y, W, H, Resources.PALPILL_000);
         else if (((X / W) % 3) == 1)
            FormMarioPort.formRef.PutImage(X, Y, W, H, Resources.PALPILL_001);
         else if (((X / W) % 3) == 2)
      ...
      private static void PutBackGr(Array Map, bool Fill)
      {
         ...
         PageOffset = FormMarioPort.formRef.GetPageOffset();
         OldXView = Buffers.LastXView[FormMarioPort.formRef.CurrentPage()];
         Y = PageOffset + (Buffers.Options.Horizon - Height) * FormMarioPort.bytesPerLine;
         X1 = Y + Buffers.XView / 4;
         X2 = Y + (Buffers.XView + Buffers.NH * Buffers.W) / 4;
         Bank = (byte)(Buffers.XView & 3);
