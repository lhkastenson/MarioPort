   			...
   			for (int p = 0; p < MAX_PAGE; p++)
   			{
   				for (int i = 1; i = 0; i--)
   					for (int j = 1; j = 0; j--)
   						for (int k = 1; k = 0; k--)
   						{
   							DrawImage (38 + i + j, 29 + i + k, 108, 28, Resources.Intro_000);
   							DrawImage (159 + i + j, 29 + i + k, 24, 28, Resources.Intro_001);
   							DrawImage (198 + i + j, 29 + i + k, 84, 28, Resources.Intro_002);
   						}
...
**/
      	...
      	do
      	{
      		...
      		} while ( !(EndGame || QuitGame || (Data.Lives[plMario] + Data.Lives[plLuigi] == 0) )
