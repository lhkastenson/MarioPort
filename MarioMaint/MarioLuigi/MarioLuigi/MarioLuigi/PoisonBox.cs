...
namespace MarioLuigi
{
   class PoisonBox : Collectable
   {
      ...
      public PoisonBox(Level level, Vector2 position, int x, int y)
      ...
      public override void OnCollected(Player collectedBy)
      {
             ...
             && collectedBy.Position.X + Tile.Width / 2 > this.Position.X)
         {
            ...
            {
               isActive = false;
               Level.AddCollectable(new Poison(Level, new Vector2(Position.X - origin.X, Position.Y - origin.Y), X, Y));
