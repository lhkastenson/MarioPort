...
namespace MarioLuigi
{
   class PiranhaPlant : Enemy
   {
      ...
      private new const float MoveSpeed = 35.0f;
      private bool top = false;

      public PiranhaPlant(Level level, Vector2 position, SpriteEffects effects, int wait)
      ...
      {
         string spritePath = "Sprites/Plant/";
         Texture2D a = null;

         // Load animations.
         a = Level.Content.Load<Texture2D>(spritePath + "PPLANT000");
      ...
      public override void Update(GameTime gameTime)
      {
         ...
         else
         {
            ...
            if (waitTime > WAIT_TIME)
            {
               ...
               else if (Velocity == Vector2.Zero && top)
               {
                  Velocity = new Vector2(0.0f, MoveSpeed);
                  waitTime = 0.0f;
                  top = false;
      ...
      {
         sprite.Draw(gameTime, spriteBatch, new Vector2(Position.X + (Tile.Width / 2f), Position.Y),
